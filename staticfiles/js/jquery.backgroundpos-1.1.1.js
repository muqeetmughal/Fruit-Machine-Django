...
function parseBackgroundPosition(value) {
	let bgPos = (value || '').split(/ /);
	let presets = {center: '50%', left: '0%', right: '100%', top: '0%', bottom: '100%'};
	let decodePos = function(index) {
		let pos = (presets[bgPos[index]] || bgPos[index] || '50%').
			match(/^([+-]=)?([+-]?\d+(\.\d*)?)(.*)$/);
...
function initBackgroundPosition(tween) {
	...
	for (let i = 0; i < tween.end.length; i++) {
